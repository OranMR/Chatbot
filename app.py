...
def rerank_results(query, chunks, distances):
    """Rerank chunks using a hybrid scoring approach"""
...
def chat():
    try:
        ...
        # Rerank the results
        reranked_texts = rerank_results(user_query, relevant_texts, chunk_distances)
        
        # Deduplicate to remove very similar chunks
        unique_texts = deduplicate_chunks(reranked_texts)
        
        # Limit context size (top 5 chunks after reranking and deduplication)
        top_chunks = unique_texts[:10]
